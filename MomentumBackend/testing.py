...
class UserTestCase(TestCase):
    ...
    def setUp(self):
        trainer: Trainer = Trainer.objects.create(
            first_name="Erik",
            last_name="Prescher",
            username="DerTrainer",
...
class PlanTestCase(TestCase):
    ...
    def setUp(self):
        ...
        ExerciseInPlan.objects.create(
            date="monday", sets=5, repeats_per_set=10, exercise=ex, plan=ts
        )
        user.plan = ts
        user.save()
    ...
    def test_if_exists(self):
        self.assertTrue(
            TrainingSchedule.objects.filter(trainer=self.trainer_id).exists()
        )
        self.assertTrue(
            ExerciseInPlan.objects.filter(exercise=self.ex_id, plan=self.ts_id).exists()
        )
    ...
    def test_if_related_deletes_work(self):
        ...
        self.assertFalse(
            ExerciseInPlan.objects.filter(exercise=self.ex_id, plan=self.ts_id)
        )
        # recreate data
        Exercise.objects.create(
            id=1, title="Kniebeuge", description={"de": "Gehe in die Knie, achte..."}
...
class getUsersAndTrainersTestCase(TestCase):
    ...
    def setUp(self) -> None:
        self.admin: Admin = Admin.objects.create(
            first_name="Erik",
            last_name="Prescher",
            username="DerAdmin",
            password="Password1234",
        ...
        User.objects.create(
            ...
            last_name="nachname",
            username="user10",
            email_address="user10@users.com",
            trainer=self.trainers[1],
            password="pswd22",
        )
    ...
    def test_methods(self):
        ...
        self.assertEquals(
            response.data.get("data").get("users"),
            UserHandler.get_users_data_for_upper(
                User.objects.filter(trainer=self.trainers[0])
            ),
        )
        # admin getting user of specific trainer
        request = ViewSupport.setup_request(
            {"Session-Token": token1}, {"id": self.trainers[1].id}
        )
        ...
        self.assertEquals(
            response.data.get("data").get("users"),
            UserHandler.get_users_data_for_upper(
                User.objects.filter(trainer=self.trainers[1])
            ),
        )
        ...
        self.assertTrue(response.data.get("success"))
        self.assertEquals(
            response.data.get("data").get("trainers"),
            TrainerHandler.get_trainers_data(Trainer.objects.all()),
        )
        ...
        request = ViewSupport.setup_request(
            {"Session-Token": token2}, {"id": self.trainers[1].id}
        )
        response = GetUsersOfTrainerView.post(GetUsersOfTrainerView, request)
        self.assertFalse(response.data.get("success"))
        # user not allowed to get trainers users
        request = ViewSupport.setup_request(
            {"Session-Token": token4}, {"id": self.trainers[1].id}
        )
        response = GetUsersOfTrainerView.post(GetUsersOfTrainerView, request)
        self.assertFalse(response.data.get("success"))
...
class AchievementTestCase(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.trainer: User = Trainer.objects.create(
            first_name="Erik",
            last_name="Prescher",
            username="DerTrainer",
            email_address="prescher-erik@web.de",
        ...
        self.user1: User = User.objects.create(
            first_name="Erik",
            last_name="Prescher",
            username="DeadlyFarts",
        ...
        self.user2: User = User.objects.create(
            first_name="Jannis",
            last_name="Bauer",
            username="jbad",
            trainer=self.trainer,
    ...
    def test_get_achievements(self):
        ...
        request = ViewSupport.setup_request(
            {
                "Session-Token": JwToken.create_session_token(
                    self.trainer.username, "trainer"
                )
            },
            {},
        )
    ...
    def test_reload_friends(self):
        ...
        self.assertTrue(response.data.get("success"))
        self.assertEquals(
            response.data.get("data").get("achievements"),
            {
                "name": "havingFriends",
                "title": "Freundschaft!",
                "description": "habe einen Freund",
...
class LevelTestCase(TestCase):
    ...
    def test_level(self):
        ...
        # invalid token
        request = ViewSupport.setup_request(
            {"Session-Token": "ìnvalid"}, {"username": self.user2.username}
        )
        response = GetUserLevelView.post(GetUserLevelView, request)
        self.assertFalse(response.data.get("success"))
...
class HandlingInvitesTestCase(TestCase):
    def setUp(self) -> None:
        ...
        self.trainer = trainer
        trainer2: Trainer = Trainer.objects.create(
            first_name="Erik",
            last_name="Prescher",
            username="DerAndereTrainer",
            email_address="prescher-erik@web.de",
        ...
        token = JwToken.create_new_user_token(
            trainer.username, "Jannis", "Bauer", "jannis@test.de", "user"
        )
        self.ot1: OpenToken = OpenToken.objects.create(
            token=token,
            email="jannis@test.de",
            first_name="Jannis",
            last_name="Bauer",
            creator=trainer.username,
        )
        token = JwToken.create_new_user_token(
            trainer2.username, "Julian", "Imhof", "julian@test.de", "user"
        )
        self.ot2: OpenToken = OpenToken.objects.create(
            token=token,
            email="julian@test.de",
            first_name="Julian",
            last_name="Imhof",
            creator=trainer2.username,
        )
    ...
    def test_get(self):
        ...
        self.assertTrue(response.data.get("success"))
        self.assertEquals(
            response.data.get("data").get("invited"),
            InvitationsHandler.get_invited_data(
                [
                    self.ot1,
                ]
            ),
    ...
    def test_invalidate(self):
        # valid
        request = ViewSupport.setup_request(
            {"Session-Token": self.token}, {"id": self.ot1.id}
        )
        ...
        request = ViewSupport.setup_request(
            {"Session-Token": self.token}, {"id": self.ot2.id}
        )
        response = InvalidateInviteView.post(InvalidateInviteView, request)
        self.assertFalse(response.data.get("success"))
        ...
        # invalid token
        request = ViewSupport.setup_request(
            {"Session-Token": "invalid"}, {"id": self.ot2.id}
        )
        response = InvalidateInviteView.post(InvalidateInviteView, request)
        self.assertFalse(response.data.get("success"))
        # missing arguments
...
class ProfileTestCase(TestCase):
    def setUp(self) -> None:
        trainer: Trainer = Trainer.objects.create(
            ...
            username="DerTrainer",
            email_address="prescher-erik@web.de",
            password=str(hashlib.sha3_256("Passwort".encode("utf8")).hexdigest()),
        )
        self.trainer_id = trainer.id
        user1: User = User.objects.create(
            first_name="Erik",
    ...
    def test_change_username(self):
        ...
        # triner
        request = ViewSupport.setup_request(
            {"Session-Token": self.token1}, {"username": "neuerName"}
        )
        ...
        # user
        request = ViewSupport.setup_request(
            {"Session-Token": self.token2}, {"username": "coolerName"}
        )
        response = ChangeUsernameView.post(ChangeUsernameView, request)
        self.assertTrue(response.data.get("success"))
        ...
        # invalid token
        request = ViewSupport.setup_request(
            {"Session-Token": "invalid"}, {"username": "coolerName"}
        )
        response = ChangeUsernameView.post(ChangeUsernameView, request)
        self.assertFalse(response.data.get("success"))
        # missing arguments
    ...
    def test_change_password(self):
        ...
        # trainer
        request = ViewSupport.setup_request(
            {"Session-Token": self.token1},
            {"password": "Passwort", "new_password": "pswd_new"},
        )
        ...
        self.assertEqual(
            trainer.password,
            str(hashlib.sha3_256("pswd_new".encode("utf8")).hexdigest()),
        )
        # user
        request = ViewSupport.setup_request(
            {"Session-Token": self.token2},
            {"password": "passwd", "new_password": "neue1234"},
        )
        response = ChangePasswordView.post(ChangePasswordView, request)
        self.assertTrue(response.data.get("success"))
        user1: User = User.objects.get(id=self.user1_id)
        self.assertEqual(
            user1.password, str(hashlib.sha3_256("neue1234".encode("utf8")).hexdigest())
        )
        # invalid
        # wrong password
        request = ViewSupport.setup_request(
            {"Session-Token": self.token3},
            {"password": "wrong", "new_password": "neverReached"},
        )
        response = ChangePasswordView.post(ChangePasswordView, request)
        self.assertFalse(response.data.get("success"))
        user2: User = User.objects.get(id=self.user2_id)
        self.assertEqual(
            user2.password, str(hashlib.sha3_256("passwdyo".encode("utf8")).hexdigest())
        )
        # invalid token
        request = ViewSupport.setup_request(
        ...
        self.assertEquals(response.data.get("data").get("header"), ["Session-Token"])
        self.assertEquals(
            response.data.get("data").get("data"), ["password", "new_password"]
        )
    ...
    def test_change_motivation(self):
        ...
        request = ViewSupport.setup_request(
            {"Session-Token": self.token3},
            {"motivation": "Nieder mit der Schwerkraft, lang lebe der Leichtsinn"},
        )
        response = ChangeMotivationView.post(ChangeMotivationView, request)
        self.assertTrue(response.data.get("success"))
        user2: User = User.objects.get(id=self.user2_id)
        self.assertEqual(
            user2.motivation, "Nieder mit der Schwerkraft, lang lebe der Leichtsinn"
        )
        # invalid
    ...
    def test_profile_data(self):
        ...
        request = ViewSupport.setup_request(
            {"Session-Token": self.token1}, {"telephone": "015712251102"}
        )
        ...
        request = ViewSupport.setup_request(
            {"Session-Token": self.token1}, {"academia": "dr. nat"}
        )
        ...
        self.assertEqual(
            response.data.get("data").get("address"),
            "Straße 4, 64287 Darmstadt, Deutschland",
        )
        ...
        request = ViewSupport.setup_request(
            {"Session-Token": self.token2}, {"telephone": "015712251102"}
        )
        response = ChangeTrainerTelephoneView.post(ChangeTrainerTelephoneView, request)
        self.assertFalse(response.data.get("success"))
        # invalid token
        request = ViewSupport.setup_request(
            {"Session-Token": "invalid"}, {"telephone": "015712251102"}
        )
        response = ChangeTrainerTelephoneView.post(ChangeTrainerTelephoneView, request)
        self.assertFalse(response.data.get("success"))
        # missing arguments
        ...
        request = ViewSupport.setup_request(
            {"Session-Token": self.token2}, {"academia": "dr. nat"}
        )
        response = ChangeTrainerAcademiaView.post(ChangeTrainerAcademiaView, request)
        self.assertFalse(response.data.get("success"))
        # invalid token
        request = ViewSupport.setup_request(
            {"Session-Token": "invalid"}, {"academia": "dr. nat"}
        )
        response = ChangeTrainerAcademiaView.post(ChangeTrainerAcademiaView, request)
        self.assertFalse(response.data.get("success"))
        # missing arguments
        ...
        self.assertEquals(response.data.get("data").get("header"), ["Session-Token"])
        self.assertEquals(
            response.data.get("data").get("data"),
...
class TestUserViews(TestCase):
    ...
    def setUp(self):
        ...
        user: User = User.objects.create(
            ...
            trainer=trainer,
            email_address="prescher-erik@web.de",
            password=str(hashlib.sha3_256("Password1234".encode("utf8")).hexdigest()),
        )
        admin: Admin = Admin.objects.create(
            first_name="Erik",
            last_name="Prescher",
    ...
    def test_login(self):
        # correct
        request = ViewSupport.setup_request(
            {}, {"username": "DeadlyFarts", "password": "Password1234"}
        )
        ...
        # invalid username
        request = ViewSupport.setup_request(
            {}, {"username": "cooleKids", "password": "Password1234"}
        )
        response = LoginView.post(LoginView, request)
        self.assertFalse(response.data.get("success"))
        # invalid pasword
        request = ViewSupport.setup_request(
            {}, {"username": "DeadlyFarts", "password": "wrong"}
        )
        response = LoginView.post(LoginView, request)
        self.assertFalse(response.data.get("success"))
        # missing arguments
        ...
        self.assertEquals(
            response.data.get("data").get("data"), ["username", "password"]
        )
    ...
    def test_register(self):
        ...
        if self.new_user_token == None:
            trainer: Trainer = Trainer.objects.get(id=self.trainer_id)
            self.new_user_token = JwToken.create_new_user_token(
                trainer.username, "Jannis", "Bauer", "bptestmail52@gmail.com", "user"
            )
            OpenToken.objects.create(
                token=self.new_user_token,
                email="bptestmail52@gmail.com",
                first_name="Jannis",
                last_name="Bauer",
                creator=trainer.username,
            )
        ...
        if self.new_trainer_token == None:
            ...
            self.new_trainer_token = JwToken.create_new_user_token(
                admin.username, "Jannis", "Bauer", "bptestmail52@gmail.com", "trainer"
            )
            OpenToken.objects.create(
                token=self.new_trainer_token,
                email="bptestmail52@gmail.com",
                first_name="Jannis",
                last_name="Bauer",
                creator=trainer.username,
    ...
    def test_createUser(self):
        ...
        # create user
        request = ViewSupport.setup_request(
            {"Session-Token": self.trainer_token},
            {
                "first_name": "Jannis",
                "last_name": "Bauer",
        ...
        # create trainer
        request = ViewSupport.setup_request(
            {"Session-Token": self.admin_token},
            {
                "first_name": "Jannis",
                "last_name": "Bauer",
                "email_address": "bptestmail52@gmail.com",
    ...
    def test_auth(self):
        ...
        if self.user_refresh_token == None:
            self.user_refresh_token = JwToken.create_refresh_token(
                "DeadlyFarts", "user", True
            )
        request = ViewSupport.setup_request(
    ...
    def test_logoutAllDevices(self):
        ...
        self.assertTrue(
            JwToken.check_refresh_token(self.user_refresh_token).get("valid")
        )
        request = ViewSupport.setup_request({"Session-Token": self.user_token}, {})
        time.sleep(10)
...
class TestExerciseView(TestCase):
    ...
    def test_get(self):
        # valid exercise
        request = ViewSupport.setup_request(
            {"Session-Token": self.trainer_token}, {"id": self.ex_id}
        )
        ...
        # invalid exercise
        request = ViewSupport.setup_request(
            {"Session-Token": self.trainer_token}, {"id": 2543}
        )
        response = GetExerciseView.post(GetExerciseView, request)
        self.assertFalse(response.data.get("success"))
        # admin not allowed
    ...
    def test_get_list(self):
        ...
        self.assertTrue(response.data.get("success"))
        self.assertTrue(
            len(response.data.get("data").get("exercises"))
            == len(Exercise.objects.all())
        )
        # invalid
        # user not allowed
        request = ViewSupport.setup_request({"Session-Token": self.user_token}, {})
...
class TestPlanView(TestCase):
    ...
    def test_create_new(self):
        ...
        self.assertTrue(
            TrainingSchedule.objects.filter(
                id=int(response.data.get("data").get("plan_id"))
            ).exists()
        )
        # invalid
        # user not allowed
    ...
    def test_add_user(self):
        ...
        TrainingSchedule.objects.create(
            name="addtouser_plan", trainer=Trainer.objects.get(id=self.trainer_id)
        )
        ...
        user.save()
        request = ViewSupport.setup_request(
            {"Session-Token": self.trainer_token}, {"plan": self.ts_id, "user": "jbadV"}
        )
        ...
        user.save()
        request = ViewSupport.setup_request(
            {"Session-Token": self.trainer_id}, {"plan": -1, "user": "DeadlyFarts"}
        )
        response = AddPlanToUserView.post(AddPlanToUserView, request)
        self.assertFalse(response.data.get("success"))
        user = User.objects.get(username="DeadlyFarts")
        ...
        request = ViewSupport.setup_request(
            {"Session-Token": self.user_token},
            {"plan": self.ts_id, "user": "DeadlyFarts"},
        )
        response = AddPlanToUserView.post(AddPlanToUserView, request)
        self.assertFalse(response.data.get("success"))
        user = User.objects.get(username="DeadlyFarts")
        ...
        # invalid token
        request = ViewSupport.setup_request(
            {"Session-Token": "invalid"}, {"plan": self.ts_id, "user": "DeadlyFarts"}
        )
        response = AddPlanToUserView.post(AddPlanToUserView, request)
        self.assertFalse(response.data.get("success"))
        # missing arguments
    ...
    def test_get_list(self):
        ...
        self.assertTrue(response.data.get("success"))
        self.assertEquals(
            len(response.data.get("data").get("plans")),
            len(TrainingSchedule.objects.filter(trainer=self.trainer_id)),
        )
        # invalid
        # user not allowed to
        request = ViewSupport.setup_request({"Session-Token": self.user_token}, {})
    ...
    def test_get(self):
        # valid
        request = ViewSupport.setup_request(
            {"Session-Token": self.trainer_token}, {"plan": self.ts_id}
        )
        ...
        self.assertEquals(
            len(response.data.get("data").get("exercises")),
            len(ExerciseInPlan.objects.filter(plan=self.ts_id)),
        )
        # invalid
        # invalid plan
        request = ViewSupport.setup_request(
            {"Session-Token": self.trainer_token}, {"plan": -1}
        )
        response = ShowPlanView.post(ShowPlanView, request)
        self.assertFalse(response.data.get("success"))
        # user not allowed
        request = ViewSupport.setup_request(
            {"Session-Token": self.user_token}, {"plan": self.ts_id}
        )
        response = ShowPlanView.post(ShowPlanView, request)
        self.assertFalse(response.data.get("success"))
        # invalid token
        request = ViewSupport.setup_request(
            {"Session-Token": "invalid"}, {"plan": self.ts_id}
        )
        response = ShowPlanView.post(ShowPlanView, request)
        self.assertFalse(response.data.get("success"))
        # missing arguments
    ...
    def test_get_for_user(self):
        ...
        ts: TrainingSchedule = TrainingSchedule.objects.create(
            name="getfromuser_plan", trainer=Trainer.objects.get(id=self.trainer_id)
        )
        # as user
        if user.plan == None:
            user.plan = ts
        ...
        self.assertEquals(
            len(response.data.get("data").get("exercises")),
            len(ExerciseInPlan.objects.filter(plan=ts.id)),
        )
        # as trainer
        request = ViewSupport.setup_request(
            {"Session-Token": self.trainer_token}, {"username": user.username}
        )
        response = GetPlanOfUser.post(GetPlanOfUser, request)
        self.assertTrue(response.data.get("success"))
        self.assertEquals(
        ...
        request = ViewSupport.setup_request(
            {"Session-Token": "invalid"}, {"username": user.username}
        )
        response = GetPlanOfUser.post(GetPlanOfUser, request)
        self.assertFalse(response.data.get("success"))
        # missing arguments
    ...
    def test_delete(self):
        # valid
        ts: TrainingSchedule = TrainingSchedule.objects.create(
            name="delete_plan", trainer=Trainer.objects.get(id=self.trainer_id)
        )
        request = ViewSupport.setup_request(
            {"Session-Token": self.trainer_token}, {"id": ts.id}
        )
        ...
        # invalid plan
        request = ViewSupport.setup_request(
            {"Session-Token": self.trainer_token}, {"id": -1}
        )
        response = DeletePlanView.post(DeletePlanView, request)
        self.assertFalse(response.data.get("success"))
        ts = TrainingSchedule.objects.create(
            name="delete_plan", trainer=Trainer.objects.get(id=self.trainer_id)
        )
        # user not allowed
        request = ViewSupport.setup_request(
            {"Session-Token": self.user_token}, {"id": ts.id}
        )
        response = DeletePlanView.post(DeletePlanView, request)
        self.assertFalse(response.data.get("success"))
...
class TestLeaderboardView(TestCase):
    ...
    def test_get(self):
        ...
        # as trainer
        request = ViewSupport.setup_request(
            {"Session-Token": self.trainer_token}, {"count": 3}
        )
        ...
        # as user
        request = ViewSupport.setup_request(
            {"Session-Token": self.user_token}, {"count": 3}
        )
        response = ListLeaderboardView.post(ListLeaderboardView, request)
        self.assertTrue(response.data.get("success"))
        leaderboard = []
...
class TestDoneExercise(TestCase):
    ...
    def setUp(self) -> None:
        ...
        ts: TrainingSchedule = TrainingSchedule.objects.create(trainer=trainer)
        exip: ExerciseInPlan = ExerciseInPlan.objects.create(
            date="monday", sets=5, repeats_per_set=10, exercise=self.ex, plan=ts
        )
        ...
        self.admin_token = JwToken.create_session_token(admin.username, "admin")
        Leaderboard.objects.create(
            user=User.objects.get(username=self.user.username), score=0
        )
    ...
    def test_get_done(self):  # not working, issue with method in tests
        ...
        # as trainer
        request = ViewSupport.setup_request(
            {"Session-Token": self.trainer_token}, {"user": self.user.username}
        )
        ...
        request = ViewSupport.setup_request(
            {"Session-Token": self.trainer_token}, {"user": "unknown"}
        )
        response = GetDoneExercisesView.post(GetDoneExercisesView, request)
        self.assertFalse(response.data.get("success"))
        # user cant call trainer method
        request = ViewSupport.setup_request(
            {"Session-Token": self.user_token}, {"user": self.user.username}
        )
        response = GetDoneExercisesView.post(GetDoneExercisesView, request)
        self.assertFalse(response.data.get("success"))
        # admin cant call trainer method
        request = ViewSupport.setup_request(
            {"Session-Token": self.admin_token}, {"user": self.user.username}
        )
        response = GetDoneExercisesView.post(GetDoneExercisesView, request)
        self.assertFalse(response.data.get("success"))
        # invalid token for trainer method
        request = ViewSupport.setup_request(
            {"Session-Token": "invalid"}, {"user": self.user.username}
        )
        response = GetDoneExercisesView.post(GetDoneExercisesView, request)
        self.assertFalse(response.data.get("success"))
...
class TestFriendSystem(TestCase):
    ...
    def test_system(self):
        ...
        # user1 adds user2
        request = ViewSupport.setup_request(
            {"Session-Token": self.token3}, {"username": "user2"}
        )
        response = AddFriendView.post(AddFriendView, request)
        self.assertTrue(response.data.get("success"))
        self.assertTrue(
            Friends.objects.filter(
                friend1=self.users[0], friend2=self.users[1], accepted=False
            ).exists()
        )
        # user2 adds user1
        request = ViewSupport.setup_request(
            {"Session-Token": self.token4}, {"username": "user1"}
        )
        response = AddFriendView.post(AddFriendView, request)
        self.assertTrue(response.data.get("success"))
        self.assertTrue(
            Friends.objects.filter(
                friend1=self.users[1], friend2=self.users[0], accepted=False
            ).exists()
        )
        # user1 adds user3
        request = ViewSupport.setup_request(
            {"Session-Token": self.token3}, {"username": "user3"}
        )
        response = AddFriendView.post(AddFriendView, request)
        self.assertTrue(response.data.get("success"))
        self.assertTrue(
            Friends.objects.filter(
                friend1=self.users[0], friend2=self.users[2], accepted=False
            ).exists()
        )
        # user1 get requests
        request = ViewSupport.setup_request({"Session-Token": self.token3}, {})
        ...
        self.assertEquals(
            response.data.get("data").get("requests"),
            FriendHandler.get_requests(self.users[0]),
        )
        # user1 get pending
        request = ViewSupport.setup_request({"Session-Token": self.token3}, {})
        ...
        self.assertEquals(
            response.data.get("data").get("pending"),
            FriendHandler.get_pending_requests(self.users[0]),
        )
        # user1 accepts from user2
        id = Friends.objects.get(
            friend1=self.users[1], friend2=self.users[0], accepted=False
        ).id
        ...
        self.assertTrue(
            Friends.objects.filter(
                friend1=self.users[1], friend2=self.users[0], accepted=True
            ).exists()
        )
        self.assertFalse(
            Friends.objects.filter(
                friend1=self.users[0], friend2=self.users[1]
            ).exists()
        )
        ...
        self.assertFalse(
            Friends.objects.filter(
                friend1=self.users[0], friend2=self.users[2]
            ).exists()
        )
        # user1 get friends
        request = ViewSupport.setup_request({"Session-Token": self.token3}, {})
        ...
        self.assertEquals(
            response.data.get("data").get("friends"),
            FriendHandler.get_friends(self.users[0]),
        )
        ...
        self.assertFalse(
            Friends.objects.filter(
                friend1=self.users[1], friend2=self.users[0]
            ).exists()
        )
        # invalid
        ...
        # add
        request = ViewSupport.setup_request(
            {"Session-Token": self.token1}, {"username": "user2"}
        )
        response = AddFriendView.post(AddFriendView, request)
        self.assertFalse(response.data.get("success"))
        # requests
        ...
        # add
        request = ViewSupport.setup_request(
            {"Session-Token": self.token2}, {"username": "user2"}
        )
        response = AddFriendView.post(AddFriendView, request)
        self.assertFalse(response.data.get("success"))
        # requests
        ...
        # add
        request = ViewSupport.setup_request(
            {"Session-Token": "invalid"}, {"username": "user2"}
        )
        response = AddFriendView.post(AddFriendView, request)
        self.assertFalse(response.data.get("success"))
        # requests
    ...
    def test_pattern_search(self):
        ...
        # as admin
        request = ViewSupport.setup_request(
            {"Session-Token": self.token1}, {"search": "user1"}
        )
        response = SearchUserView.post(SearchUserView, request)
        self.assertTrue(response.data.get("success"))
        self.assertEquals(
            response.data.get("data").get("users"),
            UserHandler.get_users_data(
                [
                    User.objects.get(username="user1"),
                    User.objects.get(username="user10"),
                ]
            ),
        )
        # as trainer
        request = ViewSupport.setup_request(
            {"Session-Token": self.token2}, {"search": "user1"}
        )
        response = SearchUserView.post(SearchUserView, request)
        self.assertTrue(response.data.get("success"))
        self.assertEquals(
        ...
        # as user
        request = ViewSupport.setup_request(
            {"Session-Token": self.token3}, {"search": "user1"}
        )
        response = SearchUserView.post(SearchUserView, request)
        self.assertTrue(response.data.get("success"))
        self.assertEquals(
            response.data.get("data").get("users"),
            UserHandler.get_users_data([User.objects.get(username="user10")]),
        )
        # empty
        request = ViewSupport.setup_request(
            {"Session-Token": self.token4}, {"search": "del"}
        )
        response = SearchUserView.post(SearchUserView, request)
        self.assertTrue(response.data.get("success"))
        self.assertEquals(
            response.data.get("data").get("users"), UserHandler.get_users_data([])
        )
        # invalid
        # invalid token
        request = ViewSupport.setup_request(
            {"Session-Token": "invalid"}, {"search": "del"}
        )
        response = SearchUserView.post(SearchUserView, request)
        self.assertFalse(response.data.get("success"))
        # missing arguments
    ...
    def test_get_list(self):
        ...
        self.assertEquals(
            response.data.get("data").get("users"),
            UserHandler.get_users_data(User.objects.all()),
        )
        # as trainer
        request = ViewSupport.setup_request({"Session-Token": self.token2}, {})
        response = GetListOfUsers.get(SearchUserView, request)
        ...
        self.assertEquals(
            response.data.get("data").get("users"),
            UserHandler.get_users_data(User.objects.all().exclude(username="user1")),
        )
        # invalid
        # invalid token
        request = ViewSupport.setup_request({"Session-Token": "invalid"}, {})
...
class TestResetPassword(TestCase):
    ...
    def test_send(self):
        ...
        # user
        request = ViewSupport.setup_request(
            {}, {"username": "derNutzer", "url": "www.test/#/"}
        )
    ...
    def test_change(self):
        ...
        # user
        request = ViewSupport.setup_request(
            {}, {"reset_token": self.token3, "new_password": "newFancy"}
        )
        response = SetPasswordResetEmailView.post(SetPasswordResetEmailView, request)
        self.assertTrue(response.data.get("success"))
        user: User = User.objects.get(id=self.user_id)
        self.assertEquals(
            user.password, str(hashlib.sha3_256("newFancy".encode("utf8")).hexdigest())
        )
        """not implemented yet
        #trainer
        request = ViewSupport.setup_request({}, {
        ...
        # invalid token
        request = ViewSupport.setup_request(
            {}, {"reset_token": "invalid", "new_password": "newFancy"}
        )
        response = SetPasswordResetEmailView.post(SetPasswordResetEmailView, request)
        self.assertFalse(response.data.get("success"))
        # missing arguments
        ...
        self.assertEquals(response.data.get("data").get("header"), [])
        self.assertEquals(
            response.data.get("data").get("data"), ["reset_token", "new_password"]
        )
...
class TestMedals(TestCase):
    ...
    def setUp(self) -> None:
        ...
        UserMedalInExercise.objects.create(
            user=user2, gold=1, silver=2, bronze=4, exercise=ex2
        )
    ...
    def test_get(self):
        ...
        self.assertTrue(response.data.get("success"))
        self.assertEquals(
            response.data.get("data").get("medals"),
            [
                {"exercise": "Kniebeuge", "gold": 2, "silver": 5, "bronze": 0},
                {"exercise": "Liegestütze", "gold": 4, "silver": 0, "bronze": 3},
            ],
        )
        request = ViewSupport.setup_request({"Session-Token": self.token3}, {})
        response = GetMedals.get(GetMedals, request)
        ...
        self.assertEquals(
            response.data.get("data").get("medals"),
            [
                {"exercise": "Kniebeuge", "gold": 6, "silver": 1, "bronze": 0},
                {"exercise": "Liegestütze", "gold": 1, "silver": 2, "bronze": 4},
            ],
        )
        # invalid
        # trainer not allowed
...
class TestProfileOfFriends(TestCase):
    ...
    def setUp(self) -> None:
        ...
        user3: User = User.objects.create(
            first_name="Jannis",
            last_name="Bauer",
            username="jbadV",
            trainer=trainer,
    ...
    def test(self):
        # valid
        request = ViewSupport.setup_request(
            {"Session-Token": self.token2}, {"username": "jbad"}
        )
        ...
        self.assertEquals(
            response.data.get("data"),
            {
                ...
                "days": 0,
                "flame_height": 0.0,
                "last_achievements": [{"name": "streak", "icon": None}],
            },
        )
        request = ViewSupport.setup_request(
            {"Session-Token": self.token3}, {"username": "DeadlyFarts"}
        )
        response = GetProfileOfFriendView.post(GetProfileOfFriendView, request)
        self.assertTrue(response.data.get("success"))
        self.assertEquals(
        ...
        # not friends
        request = ViewSupport.setup_request(
            {"Session-Token": self.token3}, {"username": "jbadV"}
        )
        response = GetProfileOfFriendView.post(GetProfileOfFriendView, request)
        self.assertFalse(response.data.get("success"))
        # not accepted
        request = ViewSupport.setup_request(
            {"Session-Token": self.token2}, {"username": "jbadV"}
        )
        response = GetProfileOfFriendView.post(GetProfileOfFriendView, request)
        self.assertFalse(response.data.get("success"))
        # trainer not allowed to
        request = ViewSupport.setup_request(
            {"Session-Token": self.token1}, {"username": "jbadV"}
        )
        response = GetProfileOfFriendView.post(GetProfileOfFriendView, request)
        self.assertFalse(response.data.get("success"))
        # invalid token
        request = ViewSupport.setup_request(
            {"Session-Token": "invalid"}, {"username": "jbadV"}
        )
        response = GetProfileOfFriendView.post(GetProfileOfFriendView, request)
        self.assertFalse(response.data.get("success"))
        # missing arguments
