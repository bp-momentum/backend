...
class ListLeaderboardView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        check = ErrorHandler.check_arguments(
            ["Session-Token"], request.headers, ["count"], request.data
        )
        ...
        if not info["account_type"] == "user":
            for i in range(0, count_of_entries):
                if i >= count_entries:
                    continue
                rank += 1
                entry = LeaderboardHandler.build_entry(
                    index=i,
        ...
        # user in bottom count_of_series
            ...
            for i in range(
                user_index - math.floor(count_of_entries / 2),
                user_index + math.ceil(count_of_entries / 2),
            ):
                rank += 1
                entry = LeaderboardHandler.build_entry(
                    index=i,
